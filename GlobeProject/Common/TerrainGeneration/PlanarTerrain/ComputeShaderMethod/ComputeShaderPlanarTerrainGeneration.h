...
struct ComputeTerrainGenInfo_OUT {
	...
	std::vector<glm::ivec4> gradientData;      // Vector to store height data
	std::vector<glm::ivec4> normalData;      // Vector to store height data
	std::vector<float> heightData;      // Vector to store height data
	...
	void free() {
		...
		gradientData.clear();
		gradientData.shrink_to_fit();
		std::vector<glm::ivec4>().swap(gradientData);

		normalData.clear();
		normalData.shrink_to_fit();
		std::vector<glm::ivec4>().swap(normalData);
...
private:
	...
	unsigned int ssboHeightOutput;
	unsigned int ssboNormalOutput;
	unsigned int ssboGradientOutput;
	...
	layout(std430, binding = 1) buffer HeightOutput {
		float heightData[];
	};

	layout(std430, binding = 2) buffer NormalOutput {
		ivec3 normalData[];
	};

	layout(std430, binding = 3) buffer GradientOutput {
		ivec3 gradientData[];
	};
	...
	void main() {
		...
		for (int i = 0; i < numOctaves; ++i) {
			...
			gradient += vec3(noise2D(x * frequency + 0.1, y * frequency) - noise2D(x * frequency - 0.1, y * frequency),
							 noise2D(x * frequency, y * frequency + 0.1) - noise2D(x * frequency, y * frequency - 0.1),
							 0.0);
		...
		heightData[idx] = noiseValue;
		normalData[idx] = ivec3(((normal + 1.0) * 0.5) * 255.0);
		gradientData[idx] = ivec3(((gradient + 1.0) * 0.5) * 255.0);
		//normalData[idx] = ivec3(255, 0, 0); // Pure red for the normal map
		//gradientData[idx] = ivec3(0, 255, 0); // Pure green for the gradient map
	}
	...
	const char* computeShaderSourceOrig = R"(
	#version 430
