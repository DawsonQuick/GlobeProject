...
private:
    ...
    Stopwatch<std::chrono::milliseconds> stopWatchA;
    Stopwatch<std::chrono::milliseconds> stopWatchB;
    ...
    const char* tesselationEvalShaderSource = R"(
        ...
        void main()
        {
            ...
            Height = packedData.r * (50 * 300);

            vec4 normal = texture(normalMap, texCoord);
            terrainNormal = (normal.rgb * 2.0) - 1.0;

            vec4 gradient = texture(gradientMap, texCoord);
            terrainGradient = (gradient.rgb * 2.0) ;

            //terrainNormal = vec3(0.0,0.0,0.0);
            //terrainGradient = vec3(0.0,0.0,0.0);
    ...
    const char* tesselationEvalShaderSourceOrig = R"(
        #version 410 core
        layout(quads, fractional_odd_spacing, ccw) in;
        uniform sampler2D heightMap;
        uniform mat4 model;
